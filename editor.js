document.addEventListener('DOMContentLoaded', () => {
  ...
  function setupEventListeners () {
    ...
    cancelBtn.addEventListener('click', () => { chrome.runtime.sendMessage({ action: 'editorCancel' }) })
    fieldSearchInput.addEventListener('focus', () => { fieldListContainer.style.display = 'block' })
    fieldSearchInput.addEventListener('blur', () => { setTimeout(() => { fieldListContainer.style.display = 'none' }, 150) })
    ...
    functionListContainer.addEventListener('click', (e) => {
      ...
      if (funcItem) {
        ...
        const funcDetails = allFunctions.find((f) => f.id === funcItem.dataset.id)
        updateFunctionInfoPanel(funcDetails)
      }
    })
    ...
    themeSelector.addEventListener('change', handleThemeChange)
        gdriveToggle?.addEventListener('change', () => {
          ...
          chrome.storage.sync.set({ gdriveDisabled: isDisabled })
          if (isDisabled) disableGDriveFeatures(false)
          else enableGDriveFeatures()
        })
        saveDriveBtn.addEventListener('click', handleGDriveSave)
        loadRevisionsBtn.addEventListener('click', handleGDriveLoadRevisions)
        revisionsSelector.addEventListener('change', handleGDriveRestoreRevision)
        ...
        searchInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); find(e.shiftKey) } else if (e.key === 'Escape') closeSearch() })
        replaceInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); replace() } else if (e.key === 'Escape') closeSearch() })
        document.getElementById('feedback-btn')?.addEventListener('click', () => { settingsDropdown.classList.add('hidden'); window.open('mailto:ideagraveyard@snippyforquickbase.com?subject=Snippy%20Feedback', '_blank') })




document.getElementById('already-donated-link')?.addEventListener('click', (e) => { e.preventDefault(); document.getElementById('unlock-modal').classList.remove('hidden'); });

document.getElementById('unlock-cancel-btn')?.addEventListener('click', () => { document.getElementById('unlock-modal').classList.add('hidden'); });




        document.getElementById('donate-btn')?.addEventListener('click', () => { settingsDropdown.classList.add('hidden'); window.open('https://buymeacoffee.com/snippyforquickbase', '_blank') })

        window.addEventListener('click', (e) => {
          if (!settingsDropdown.classList.contains('hidden') && !settingsDropdown.contains(e.target) && e.target !== settingsBtn) {
            settingsDropdown.classList.add('hidden')
          }
          if (adminDropdown && !adminDropdown.classList.contains('hidden') && !adminDropdown.contains(e.target) && e.target !== adminPanelBtn) {
            adminDropdown.classList.add('hidden')
          }
          if (donorDropdown && !donorDropdown.classList.contains('hidden') && !donorDropdown.contains(e.target) && e.target !== donorPanelBtn) {
            donorDropdown.classList.add('hidden')
          }
        })
        ...
        showCommentsToggle.addEventListener('change', (e) => {
          chrome.storage.sync.set({ showFreeloaderComments: e.target.checked })
        })
        ...
        lightThemeSelect.addEventListener('change', () => {
          chrome.storage.sync.set({ preferredLightTheme: lightThemeSelect.value })
        ...
        darkThemeSelect.addEventListener('change', () => {
          chrome.storage.sync.set({ preferredDarkTheme: darkThemeSelect.value })
          if (document.body.classList.contains('dark') && typeof cmEditor !== 'undefined') {
